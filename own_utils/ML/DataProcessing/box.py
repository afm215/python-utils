...
def randomize_box(box: 'tuple[int, int , int, int]', img_shape: 'tuple[int, int]', min_size: int, temperature: float = 0.1) -> 'tuple[int, int , int, int]':
    ...
    rnd_x = np.clip(rnd_x, 0, img_shape[1] - min_size)
    rnd_y =  np.clip(rnd_y, 0,  img_shape[0] - min_size)
    rnd_width = np.clip(rnd_width, min_size, img_shape[1] - rnd_x)
    rnd_height = np.clip(rnd_width,min_size, img_shape[0] - rnd_y)
